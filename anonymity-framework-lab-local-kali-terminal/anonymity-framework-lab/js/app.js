...
const liveFireGuide = [
  {
    title: 'Step 1 - Confirm the local Kali shell',
    hint: 'Start by verifying that the embedded terminal is the same Kali machine that is hosting this website.',
    evidence: 'Local shell verified on the hosting Kali machine.',
    commands: [
      { cmd: 'whoami', help: 'Shows the current Kali user.', matches: [/^whoami$/i] },
      { cmd: 'pwd', help: 'Shows the starting directory of the hosted website.', matches: [/^pwd$/i] },
      { cmd: 'hostname -I', help: 'Shows the IP address used by other machines to open the website.', matches: [/^hostname\s+-I$/i, /^ip\s+a/i] }
    ]
  },
  {
    title: 'Step 2 - Move to WireGuard configuration area',
    hint: 'Go to the WireGuard directory and confirm available interface files before changing routes.',
    evidence: 'WireGuard folder inspected before testing.',
    commands: [
      { cmd: 'cd /etc/wireguard', help: 'Changes terminal context to WireGuard config directory.', matches: [/^cd\s+\/etc\/wireguard\/?$/i] },
      { cmd: 'ls -la', help: 'Lists wg0/wg1/wg2 files and permissions.', matches: [/^ls(\s+-[a-zA-Z]+)*$/i] },
      { cmd: 'sudo wg show', help: 'Shows active WireGuard interfaces and peers.', matches: [/^(sudo\s+)?wg\s+show$/i] }
    ]
  },
  {
    title: 'Step 3 - Direct victim reachability test',
    hint: 'Disable the first tunnel and test the victim directly. Blue team should see the local/client source.',
    evidence: 'Direct route test executed; victim-side capture should show the local client path.',
    commands: [
      { cmd: 'sudo wg-quick down wg0', help: 'Brings down wg0 if it is already active.', matches: [/^(sudo\s+)?wg-quick\s+down\s+wg0$/i] },
      { cmd: 'curl http://10.0.40.100', help: 'Sends HTTP traffic to the victim.', matches: [/^curl\s+http:\/\/10\.0\.40\.100\/?$/i] }
    ]
  },
  {
    title: 'Step 4 - Enable VPS-1 route and test again',
    hint: 'Bring up the first WireGuard hop, then generate traffic. Capture should now show VPS-1 visibility.',
    evidence: 'VPS-1 route test executed; observable source should change to the first relay path.',
    commands: [
      { cmd: 'sudo wg-quick up wg0', help: 'Activates wg0 toward VPS-1.', matches: [/^(sudo\s+)?wg-quick\s+up\s+wg0$/i] },
      { cmd: 'ping -c 4 10.0.40.100', help: 'Generates ICMP traffic for Wireshark visibility.', matches: [/^ping(\s+-c\s+\d+)?\s+10\.0\.40\.100$/i] }
    ]
  },
  {
    title: 'Step 5 - Enable second relay path',
    hint: 'Move to the VPS-1 relay session and enable the next tunnel toward VPS-2, then run a victim request.',
    evidence: 'Second relay test executed; victim-side capture should show the VPS-2 stage.',
    commands: [
      { cmd: 'ssh book@10.0.10.26', help: 'Connects from Kali to VPS-1 for second-hop control.', matches: [/^(sudo\s+)?ssh\s+\S+@10\.0\.10\.26$/i] },
      { cmd: 'sudo wg-quick up wg1', help: 'Activates wg1 toward VPS-2 from the relay session.', matches: [/^(sudo\s+)?wg-quick\s+up\s+wg1$/i] },
      { cmd: 'curl http://10.0.40.100', help: 'Tests victim access through the second relay.', matches: [/^curl\s+http:\/\/10\.0\.40\.100\/?$/i] }
    ]
  },
  {
    title: 'Step 6 - Tor / SOCKS route validation',
    hint: 'Start or use the SOCKS/Tor path and generate one final request so the capture shows the Tor-side source.',
    evidence: 'Tor/SOCKS validation executed; final visibility should be the Tor-side node.',
    commands: [
      { cmd: 'ssh book@10.0.40.102', help: 'Connects to the Tor-side node in the lab.', matches: [/^(sudo\s+)?ssh\s+\S+@10\.0\.40\.102$/i] },
      { cmd: 'sudo ssh -D 0.0.0.0:1080 administrator@10.0.40.102', help: 'Creates the SOCKS tunnel used for Tor-style routing.', matches: [/^(sudo\s+)?ssh\s+-D\s+0\.0\.0\.0:1080\s+\S+@10\.0\.40\.102$/i] },
      { cmd: 'proxychains curl http://10.0.40.100', help: 'Sends the victim request through proxychains/SOCKS.', matches: [/^(proxychains|proxychains4)\s+curl\s+http:\/\/10\.0\.40\.100\/?$/i, /^curl\s+http:\/\/10\.0\.40\.100\/?$/i] }
    ]
  }
];
...
function renderLiveGuide() {
  ...
  const step = liveFireGuide[Math.min(currentLiveGuideStep, total - 1)];
  ...
  if (complete) {
    ...
    hint.textContent = 'All guided commands have been entered. Review the Kali terminal output and Blue Team captures for evidence.';
    counter.textContent = `${total}/${total}`;
    list.innerHTML = '<div class="live-command-item done"><code>Guide complete</code><span>All command hints executed.</span></div>';
    bar.style.width = '100%';
    return;
  }
  ...
  const completedCommands = completedBefore + completedThis;
  title.textContent = step.title;
  hint.textContent = step.hint;
  counter.textContent = `${currentLiveGuideStep + 1}/${total}`;
  bar.style.width = `${Math.round((completedCommands / totalCommands) * 100)}%`;
  ...
  list.innerHTML = step.commands.map((item, idx) => {
    ...
    const stateLabel = item.done ? 'DONE' : (idx === nextIndex ? 'RUN NEXT' : 'WAIT');
    return `<div class="live-command-item ${state}">
      <div><code>${escapeHtml(item.cmd)}</code><span>${escapeHtml(item.help)}</span></div>
      <b>${stateLabel}</b>
    </div>`;
  }).join('');
}

function completeCurrentLiveCommand(cmd, commandIndex) {
  const step = liveFireGuide[currentLiveGuideStep];
  const item = step.commands[commandIndex];
  item.done = true;
  addLiveGuideLog(`✓ Accepted: ${cmd}`, 'ok');
  const allDone = step.commands.every(c => c.done);
  if (allDone) {
...
function processLiveCommand(raw) {
  ...
  const total = liveFireGuide.length;
  if (cmd === 'clear') {
    addLiveGuideLog('Terminal cleared. Guide progress preserved.', 'warn-text');
    return;
  }
  if (currentLiveGuideStep >= total) {
    addLiveGuideLog(`Command received after completion: ${cmd}`, 'action-text');
    return;
  }
  const step = liveFireGuide[currentLiveGuideStep];
  const matchIndex = step.commands.findIndex(item => !item.done && item.matches.some(rx => rx.test(cmd)));
  if (matchIndex >= 0) {
    completeCurrentLiveCommand(cmd, matchIndex);
    return;
  }
...
function connectLocalTerminal() {
  if (!window.io) {
    ...
    liveLine('[error] Start the app with: npm install && npm start');
    return;
  }
...
function focusManualTerminal() {
  if (initRealTerminal() && xterm) {
    xterm.focus();
    addLiveGuideLog('Terminal focused. Type the next hinted command.', 'action-text');
  }
}
